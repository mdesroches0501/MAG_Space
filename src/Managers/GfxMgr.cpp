...
GfxMgr::GfxMgr(Engine *engine)
        : Mgr(engine), oceanSurface(Ogre::Vector3::UNIT_Y, 0)
{
...
void GfxMgr::MakeSky()
{
    m_Engine->m_GfxMgr->mSceneMgr->setSkyBox(true, "Examples/MorningSkyBox");
}
...
void GfxMgr::MakeGround()
{
    ...
    //plane,
            oceanSurface, 15000, 15000, 20, 20, true, 1, 5, 5, Ogre::Vector3::UNIT_Z);

    Ogre::Entity* groundEntity = m_Engine->m_GfxMgr->mSceneMgr->createEntity("ocean");
    m_Engine->m_GfxMgr->mSceneMgr->getRootSceneNode()->createChildSceneNode()->attachObject(groundEntity);
