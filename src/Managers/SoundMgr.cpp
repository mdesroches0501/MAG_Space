...
SoundMgr::SoundMgr(Engine *eng)
        : Mgr(eng)
{
    this->m_Engine = eng;
    backgroundWaveInfo = NULL;
    context = NULL;
    device = NULL;
    isEnabled = false;
    battleSoundSource = 0;
    backgroundMusicSource = 0;
    backgroundMusicBuffer = 0;
}
...
bool SoundMgr::initWatercraftSounds()
{
    ...
    for(auto et = m_Engine->m_EntityMgr->m_Entities.begin(); et != m_Engine->m_EntityMgr->m_Entities.end(); ++et)
    {
...
void SoundMgr::LoadLevel(std::string levelLocation)
{
    syncListenerToCamera();
...
void SoundMgr::attachSelectedNodeToSoundIndex(Entity381 *ent, int index)
{
    ...
    Ogre::Vector3 pos = ent->m_Position;
...
{
    Ogre::Vector3 pos = et.m_Position;
    ...
    this->playAudioSourceIndex(et.m_AuioId);
    setSoundPosition(sourceInfo[et.m_AuioId].source, pos);
...
bool SoundMgr::registerSelection(Entity381 et, std::string filename)
{
    ...
    if(this->reserveAudio(filename, false, sid))
    {
        ...
        for(int i = 0; i < soundPerEnt; i++)
        {
            if(this->selectionSoundsDictionary[et.m_AuioId][i] == -1)
            {
                lastIndex = i;
                break;
            }
        ...
        if(lastIndex == -1)
        { //all permitted number of sounds for this type are already assigned
            std::cout << "Could not register new sound, max allowed number per entity reached" << std::endl;
            return false;
        }
        this->selectionSoundsDictionary[et.m_AuioId][lastIndex] = sid;
...
bool SoundMgr::loadAudio(std::string filename, int index)
{
    ...
    alBufferData(this->bufferInfo[index].buffer, toALFormat(this->bufferInfo[index].wave->channels, this->bufferInfo[index].wave->bitsPerSample), tmpBuf,
            this->bufferInfo[index].wave->dataSize, this->bufferInfo[index].wave->sampleRate);
    free(tmpBuf);
...
bool SoundMgr::loadStartBackground()
{
    ...
    alBufferData(this->backgroundMusicBuffer, toALFormat(this->backgroundWaveInfo->channels, this->backgroundWaveInfo->bitsPerSample), tmpBuf,
            this->backgroundWaveInfo->dataSize, this->backgroundWaveInfo->sampleRate);
    if(printError("Failed to load bufferData") < 0)
    {
        return false;
    }
...
{
    alSource3f(audioId, AL_POSITION, position.x, position.y, position.z);
    if(printError("SetSoundDisPosition::Source position") < 0)
        return false;

    alSource3f(audioId, AL_VELOCITY, velocity.x, velocity.y, velocity.z);
    if(printError("SetSoundDisPosition::Source velocity") < 0)
        return false;

    return true;
}

bool SoundMgr::setSound(ALuint audioID, Ogre::Vector3 position, Ogre::Vector3 velocity, Ogre::Vector3 direction, float maxDistance, bool playNow, bool forceRestart, float minGain)
{
    return false;
}
