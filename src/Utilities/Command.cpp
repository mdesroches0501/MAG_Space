...
void Intercept::tick(float dt)
{
    targetLocation = targetEntity->m_Position;
    Ogre::Vector3 diff = targetLocation - entity->m_Position;
    Ogre::Vector3 relVel = targetEntity->m_Velocity - entity->m_Velocity;
    float tStar;      
    tStar = diff.length()/relVel.length();
    Ogre::Vector3 interceptLocation = targetLocation + (targetEntity->m_Velocity * tStar);
    diff = interceptLocation - entity->m_Position;
    float angle = atan2(diff.z, diff.x);
    //entity->m_DesiredHeading = FixAngle(Ogre::Degree(Ogre::Radian(angle)).valueDegrees());
    //entity->m_DesiredSpeed = entity->m_MaxSpeed;
}
